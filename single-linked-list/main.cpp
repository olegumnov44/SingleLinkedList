...
public:
    ...
    SingleLinkedList(Node head, Node foot, size_t size) : head_(head), foot_(foot), size_(size) {}
    ...
    void PushBack(const Type& value) {
        foot_.next_node = new Node(value, foot_.next_node);
        ++size_;
    }
    ...
    [[nodiscard]] ConstIterator cbefore_begin() const noexcept {
        ConstIterator tmp(const_cast<Node*>(&head_));
        return tmp;
    }
    ...
    Iterator InsertAfter(ConstIterator pos, const Type& value) {
        auto curr = new Node(value, pos.node_->next_node);
        pos.node_->next_node = curr;
        ++size_;
...
private:
    Node head_; // Фиктивный узел, нужен для вставки "перед первым элементом"
    Node foot_; // Фиктивный узел, нужен для вставки "после последнего элемента"
    size_t size_ = 0;
};
...
void Test2()
{
    ...
    // Итерирование по непустому списку
    {
        ...
        *list.begin() = -1;
        ...
        // 1 Итератор, указывающий на позицию после последнего элемента, равен итератору end()
        {
            auto old_begin_copy(old_begin);
            assert((++old_begin_copy) == list.end());
        }
        // 2 Итератор, указывающий на позицию после последнего элемента, не равен итератору end()
        {
            auto old_begin_copy(old_begin);
            assert((old_begin_copy++) != list.end());
        }
